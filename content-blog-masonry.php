...
if ( 'date' != $post_style && in_array( 'date', $porto_settings['post-metas'] ) ) {
	$post_meta .= '<div class="post-meta"><span class="meta-date">' . get_the_date( esc_attr( $porto_settings['blog-date-format'] ) ) . '</span></div>';
...
elseif ( 'hover_info' == $post_style || 'hover_info2' == $post_style ) :
	...
	<div class="grid-box">
		...
		<div class="clearfix">
			<a class="btn btn-xs btn-default text-xs text-uppercase" href="<?php echo esc_url( apply_filters( 'the_permalink', get_permalink() ) ); ?>"><?php esc_html_e( '[:ua]Читати далі[:en]Read more[:ru]Читать дальше', 'porto' ); ?></a>
		</div>
	</div>
<?php endif; ?>
</article>
