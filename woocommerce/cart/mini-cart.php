...
<?php
	...
	echo '<div class="total-count">';
		...
		// echo '<span>' . sprintf( esc_html( _n( '%d item', '%d items', $items, 'woocommerce' ) ), $items ) . '</span>';
		echo '<span>'.__('Products','woocommerce').': '.$items.'</span>';
		echo '<a class="pull-right" href="' . esc_url( version_compare( $porto_woo_version, '2.5', '<' ) ? WC()->cart->get_cart_url() : wc_get_cart_url() ) . '">' . esc_html__( 'Cart', 'woocommerce' ) . '</a>';
	echo '</div>';
?>
