			...
			<div id="payment">
				...
				<ul class="wc_payment_methods payment_methods methods">
					<?php if ( version_compare( $porto_woo_version, '2.5', '<' ) ) : ?>
						...
						if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) {
							...
							echo '<li>' . esc_html__( 'Извините, похоже, что для вашего местоположения нет доступных способов оплаты. Пожалуйста, свяжитесь с нами, если вам нужна помощь или вы хотите принять альтернативные меры.', 'porto' ) . '</li>';
						}
						?>
					<?php else : ?>
						<?php
						if ( ! empty( $available_gateways ) ) {
							...
							echo '<li>' . apply_filters( 'woocommerce_no_available_payment_methods_message', __( 'Извините, похоже, что для вашего местоположения нет доступных способов оплаты. Пожалуйста, свяжитесь с нами, если вам нужна помощь или вы хотите принять альтернативные меры.', 'porto' ) ) . '</li>'; // @codingStandardsIgnoreLine
						}
						?>
					<?php endif; ?>
				</ul>
				...
				<div class="form-row">
					<?php if ( version_compare( $porto_woo_version, '2.5', '<' ) ) : ?>
						...
						<?php
						$pay_order_button_text = apply_filters( 'woocommerce_pay_order_button_text', __( 'Оплатить заказ', 'porto' ) );
