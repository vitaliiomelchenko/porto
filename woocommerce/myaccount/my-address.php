...
if ( ! wc_ship_to_billing_address_only() && $shipping_enabled ) {
	$page_title    = apply_filters( 'woocommerce_my_account_my_address_title', __( 'Мой адрес', 'porto' ) );
	...
	$get_addresses = apply_filters(
		'woocommerce_my_account_get_addresses',
		array(
			'billing' => __( 'Платёжный адрес', 'porto' ),
		),
		$customer_id
	);
}
...
<?php foreach ( $get_addresses as $name => $title ) : ?>
	...
	<div class="u-column<?php echo ( ( $col = $col * -1 ) < 0 ) ? 1 : 2; ?> col-<?php echo ( ( $oldcol = $oldcol * -1 ) < 0 ) ? 1 : 2; ?> woocommerce-Address address">
		<header class="woocommerce-Address-title title">
			...
			<a href="<?php echo esc_url( wc_get_endpoint_url( 'edit-address', $name ) ); ?>" class="edit link-with-pseudo"><?php esc_html_e( 'Edit', 'woocommerce' ); ?></a>
		</header>

		<address>
		<?php
			$address = wc_get_account_formatted_address( $name );
			echo ! $address ? esc_html_e( 'Вы еще не настроили этот тип адреса.', 'woocommerce' ) : wp_kses_post( $address );
		?>
		</address>
	</div>
