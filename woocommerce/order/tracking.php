...
$order_status_text = sprintf( __( 'Заказ #%1$s был оформлен %2$s и в настоящее время &ldquo;%3$s&rdquo;', 'porto' ), $order->get_order_number(), human_time_diff( strtotime( $order->order_date ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'porto' ), wc_get_order_status_name( $order->get_status() ) );
					...
					<div class="comment_container">
						<div class="comment-text">
							...
							<div class="description">
								<?php echo function_exists( 'porto_shortcode_format_content' ) ? porto_shortcode_format_content( wp_kses_post( $note->comment_content ) ) : wp_kses_post( $note->comment_content ); ?>
							</div>
							<div class="clear"></div>
						</div>
						<div class="clear"></div>
