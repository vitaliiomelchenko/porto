...
function nova_styles() {
  wp_enqueue_style( 'nova_bootstrap', get_template_directory_uri() . '/css/bootstrap.css' );
  wp_enqueue_style( 'nova_bootstrap_grid', get_template_directory_uri() . '/css/bootstrap-grid.min.css' );
...
function porto_css() {
	...
	wp_register_style( 'porto-style', PORTO_URI . '/style.css', array(), '1.0.42' );
	if ( is_rtl() ) {
...
function porto_scripts() {
	...
	if ( ! is_admin() && ! in_array( $GLOBALS['pagenow'], array( 'wp-login.php', 'wp-register.php' ) ) ) {
		...
		wp_register_script( 'porto-theme', PORTO_JS . '/theme' . $min_suffix . '.js', array( 'jquery' ), '1.0.22', true );
...
function custom_override_billing_fields( $fields ) {
    ...
    $fields['billing_address_1']['placeholder'] = 'Укажите информацию для доставки';
    return $fields;
}
...
include get_stylesheet_directory() . '/inc/novaposhta.php';

function theme_fetch_ajax() {
	if($_POST['fetch'] == 'np_departments' && $ref = $_POST['ref']){
		$np = new NP('028dd9d856a2c1069260207f70c72b52', 'ua', FALSE, 'curl');
		$lang = $_POST['lang'] == 'ru-RU' ? 'DescriptionRu' : 'Description';
		// $cities = $np->getCities();
		
		if($result = get_option('np_'.$ref)){
			// It's cool.
		} else {
			$result = $np->getWarehouses($ref);
		    
		    update_option('np_'.$ref, $cities, false);
		}
		
		foreach($result['data'] as $department){
		    echo '<option>'.$department[$lang].'</option>';
		}
	}
    die;
}

add_action('wp_ajax_fetch_posts', 'theme_fetch_ajax');
add_action('wp_ajax_nopriv_fetch_posts', 'theme_fetch_ajax');

// Header Login Shortcode

function header_login_shortcode(){
	$return = '<a href="'.get_permalink(get_option('woocommerce_myaccount_page_id')).'orders" class="best-price">';
	if(is_user_logged_in()){
		if(qtranxf_getLanguage() == 'en'){
			$return .= __('Account','woocommerce');
		} else {
			$return .= 'Аккаунт';
		}
	} else {
		$return .= __('Log in','woocommerce');
	}
	$return .= '</a>';
	return $return;
}

add_shortcode('header-login', 'header_login_shortcode');
...
function admin_order_list_top_bar_button( $which ) {
    ...
    if ( 'shop_order' === $typenow && 'top' === $which ) {
        ...
        <div class="alignleft actions custom">
            <a href="<?=home_url('?exportorders')?>" style="height:32px;" class="button" value=""><?php
                echo __( 'Экспорт заказов', 'woocommerce' ); ?></a>
        </div>
        <?php
    }
}
...
function custom_export_orders(){
	...
	$num = 1; foreach($orders as $order){ $num++;
		$order = wc_get_order($order);
		$order_data = $order->get_data();
		$order_name = $order_data['billing']['last_name'].' '.$order_data['billing']['first_name'];
		$order_email = $order_data['billing']['email'];
		$order_phone = $order_data['billing']['phone'];
		$order_amount = $order_data['total'];
		$order_shipping = $order_data['billing']['address_1'];
		$order_billing = $order_data['payment_method_title'];
		$order_products = '';
		
		$num = 0; foreach ( $order->get_items() as $item_id => $item ) { $num++;
		   ...
		   $newline = $num > 1 ? ', ' : '';
		   $order_products .= $newline . $name.' - x'.$quantity.' - '.$total;
		}
	...
	array_to_csv_download($list, current_time( 'Y-m-d' ).'.csv');
}

// Отображать товары со статусом "Нет в наличии" в конце списка
